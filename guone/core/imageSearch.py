...
class Indexer(object):
    ...
    def is_indexed(self, imname):
        im = self.con.execute(
            'select rowid from imlist where filename = ?',
            (imname,)
        ).fetchone()
    ...
    def get_id(self, imname):
        ...
        cur = self.con.execute(
            'insert into imlist(filename) values (?)',
            (imname,)
        )
...
class Searcher(object):
    ...
    def get_imhistogram(self, imname):
        ...
        imid = self.con.execute(
            'select rowid from imlist where filename = ?',
            (imname,)
        ).fetchone()
        s = self.con.execute(
            'select histogram from imhistograms where rowid = ?',
            imid
        ).fetchone()
    ...
    def get_filename(self, imid):
        return self.con.execute(
            'select filename from imlist where rowid = ?',
            (imid,)
        ).fetchone()[0]
