...
from scripts import imageSearch
...
def queryresults(imageurl=None):
    ...
    searcher = imageSearch.Searcher(db, voc)
    if imageurl:
        # 查询数据库并获取靠前的图像
        ...
        for dist, ndx in res:
            ...
            if 0 == i:
                if req in [1, 2, 3, 4]:
                    ...
                    rehtml += '&emsp;'
                    rehtml = rehtml + '<font color="" size="2">相似度: '.decode("utf-8") + u'%.1f' % float(simi[req - 1] * 100)  # Similarity
                    ...
                    if 1 == req:
                        ...
                        rehtml += '</h4>'
                        rehtml += '<hr>'
                        rehtml += '<h4><font color="">对该图像的最佳猜测为：&emsp;%s</font></h4>'.decode("utf-8") % getchname(bestimagename).decode('gbk').encode('utf-8').decode('utf-8')
                # elif req not in [0, 1, 2, 3, 4]:
                #     i = 1
                #     rehtml += '<hr>'
                #     rehtml += '<br>'
                #     rehtml += '<h4>其他外观类似的图像'.decode("utf-8") + '</h4>'
                #     rehtml += '<fieldset>'
                #     rehtml += '<br>'
            # if req > 4:
            #     rehtml += '<img src="/static/data/%s" width=100>' % os.path.basename(imname)
            #     rehtml += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
            req += 1
        return rehtml
